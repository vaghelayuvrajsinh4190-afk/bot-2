...
def ban_user(owner_id: str, username: str, reason: str, days: int, banned_by: str, scrim_id: str = "SQ"):
    ...
    punishments.update_one(
        {"owner_id": owner_id, "scrim_id": scrim_id, "type": "ban", "active": True},
        {"$set": {
            "owner_id": owner_id,
            "scrim_id": scrim_id,
            "username": username,
...
def unban_user(owner_id: str, scrim_id: str = None):
    """Remove an active ban. If scrim_id is None, unbans from all scrims."""
    query = {"owner_id": owner_id, "type": "ban", "active": True}
    if scrim_id:
...
def is_banned(owner_id: str, scrim_id: str = None):
    """
    Check if a user is currently banned.
    Also handles automatic expiry.
    Returns (is_banned, ban_doc or None).
    
    If scrim_id is None, checks for any active ban.
    """
    query = {
        "owner_id": owner_id,
        "type": "ban",
    ...
    ban = punishments.find_one(query)
...
def get_active_bans(scrim_id: str = None):
    """Get all currently active bans. If scrim_id is None, returns all bans."""
    query = {"type": "ban", "active": True}
    if scrim_id:
        query["scrim_id"] = scrim_id
    bans = list(punishments.find(query))
