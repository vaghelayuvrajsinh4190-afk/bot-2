...
class TeamRegistrationModal(ui.Modal, title="📋 Team Registration"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = make_embed(
            ...
            f"╭── 📋 **Roster** ──╮\n{roster_text}\n╰───────────────────╯\n\n"
            f"{Theme.THIN_SEP}\n\n"
            f"**Next:** Select your **4 squad members** from the dropdown below.\n"
...
class TeammateSelect(ui.UserSelect):
    """Dropdown to select 4 squad members."""
    
    def __init__(self, team_name, players):
        self.team_name = team_name
        self.players = players
        super().__init__(
            placeholder="🎯 Select the 4 squad members…",
            min_values=4,
            max_values=4
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user not in members:
            await interaction.response.send_message(
                embed=error_embed(
                    ...
                    f"*➤ Select yourself plus your 3 teammates.*\n\n{Theme.SEP}"
                ),
                ephemeral=True
            )
            return
        ...
        if not assigned_group:
            await interaction.followup.send(
                embed=error_embed(
                    "❌ Claim Failed",
                    f"{Theme.SEP}\n\n"
                    f"All groups filled up while processing your request.\n"
                    f"Please try again or wait for more groups.\n\n{Theme.SEP}"
                ),
                ephemeral=True
            )
            return
        ...
        # Create registration
        teammate_ids = [str(m.id) for m in members]
        ...
        group_id = assigned_group["group_id"]
        slot_num = assigned_group["current_count"]
        capacity = assigned_group["capacity"]

        # Build success embed
        member_mentions = "\n".join([f"  ◆ {m.mention}" for m in members])
        channel_id = assigned_group.get("channel_id")
        channel_link = f"<#{channel_id}>" if channel_id else "your group channel"
        ...
        success = make_embed(
            ...
            f"│\n"
            f"│  🏷️ **Team:** `{self.team_name}`\n"
            f"│  📍 **Group:** `{group_id}`\n"
            f"│  🔢 **Slot:** `#{slot_num}/{capacity}`\n"
            f"│\n"
            f"╰────────────────────────────╯\n\n"
            f"**👥 Squad:**\n{member_mentions}\n\n"
            f"{Theme.THIN_SEP}\n\n"
            f"**🎮 Match Schedule:**\n"
            f"> **Match 1:** IDP `{m1.get('idp', 'TBD')}` │ Start `{m1.get('start', 'TBD')}` │ Map `{m1.get('map', 'TBD')}`\n"
            f"> **Match 2:** IDP `{m2.get('idp', 'TBD')}` │ Start `{m2.get('start', 'TBD')}` │ Map `{m2.get('map', 'TBD')}`\n\n"
            f"📍 Join {channel_link} for updates.\n"
            f"🍀 **Best of luck!**\n\n{Theme.SEP}",
            Theme.SUCCESS,
            f"Registered at slot #{slot_num}"
        )
...
class SavedProfileView(ui.View):
    ...
    @ui.button(label="✅ Use Saved Team", style=discord.ButtonStyle.success)
    async def use_saved(self, interaction: discord.Interaction, button: ui.Button):
        ...
        embed = make_embed(
            "👥 Select Squad Members",
            f"{Theme.SEP}\n\n"
            f"Using saved team **{team_name}**\n\n"
            f"╭── 📋 **Roster** ──╮\n{roster_text}\n╰───────────────────╯\n\n"
            f"{Theme.THIN_SEP}\n\n"
            f"Select your **4 squad members** from the dropdown.\n"
            f"You must include yourself.\n\n{Theme.SEP}",
            Theme.ACCENT,
            "Using saved profile"
        )
    ...
    @ui.button(label="📝 Register New Team", style=discord.ButtonStyle.primary)
    ...
    @ui.button(label="✏️ Edit Saved Team", style=discord.ButtonStyle.secondary)
    ...
    async def register_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if profile:
            ...
            embed = make_embed(
                f"🔄 Welcome Back!",
                f"{Theme.SEP}\n\n"
                f"We found your saved team profile:\n\n"
                f"╭── 📋 **{team_name}** ──╮\n{roster_text}\n╰───────────────────────╯\n\n"
                f"{Theme.THIN_SEP}\n\n"
